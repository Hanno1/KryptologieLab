...
# RCs for aes keygeneration
RCON = [1, 2, 4, 8, 16, 32, 64, 128, 27, 54]
# mix column permutation matrix
MIX_COL = [[2, 3, 1, 1], [1, 2, 3, 1], [1, 1, 2, 3], [3, 1, 1, 2]]
# inverted mix column permutation matrix
...
def check_aes_key(keys, block_length):
    """
    validate the keys used for the aes -> has to be 11 keys and the length of each has to be equal to the block length
    """
    if len(keys) != 11:
...
def read_key_from_file(file_name):
    """
    gets a file name and returns a list of all keys inside the file. Should be 11 for aes
    """
    keys = []
...
def hex_string_to_nice_hex_string(hex_string):
    """
    converts a string of hex chars into a better format -> it adds spaces between every two chars
    """
    nice_hex_string = ""
